...
(() => {
  ...
  // The script is deliberately split between the itch.io page and the actual
  // game iframe. The parent page owns the touch UI; the game frame receives
  // commands with postMessage and dispatches them in the game's own document.
  ...
  function installGameBridge() {
    if (window.top === window.self) {
      // This is unlikely for the normal itch.io embed, but the bridge still
      // works if the game runtime is opened directly.
    }

    window.addEventListener('message', (event) => {
      if (event.origin !== OUTER_ORIGIN) return;
      const data = event.data;
      if (!data || data.source !== MESSAGE_SOURCE || !data.type) return;

      switch (data.type) {
        case 'key':
          dispatchKey(data.key, Boolean(data.down));
          break;
        case 'mouseMove':
          dispatchMouseMove(Number(data.dx) || 0, Number(data.dy) || 0);
          break;
        case 'mouseButton':
          dispatchMouseButton(data.button === 'right' ? 2 : 0, Boolean(data.down));
          break;
        case 'mousePosition':
          if (Number.isFinite(data.x) && Number.isFinite(data.y)) {
            virtualMouse.x = data.x;
            virtualMouse.y = data.y;
          }
          break;
      }
    });

    // Tell the parent frame that the bridge is installed. This also helps
    // when the iframe is recreated by itch.io.
    try {
  ...
  function dispatchKey(key, down) {
    ...
    if (target !== document) document.dispatchEvent(event);
    if (document.body && target !== document.body) document.body.dispatchEvent(event);
  }

  function dispatchMouseMove(dx, dy) {
    const canvas = document.querySelector('canvas') || document.elementFromPoint(virtualMouse.x, virtualMouse.y) || document.body;
  ...
  function dispatchMouseButton(button, down) {
    const target = document.elementFromPoint(virtualMouse.x, virtualMouse.y) || document.querySelector('canvas') || document.body;
    const type = down ? (button === 2 ? 'mousedown' : 'mousedown') : 'mouseup';
  ...
  function installControllerUI() {
    ...
    style.textContent = `
      ...
      #${UI_ID} button,#${UI_ID} .cmc-pad{
        border:1px solid rgba(255,255,255,.28);background:rgba(20,20,25,.68);
        color:#fff;backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);
        box-shadow:0 3px 14px rgba(0,0,0,.3);font-weight:700;
      }
      ...
      #${UI_ID} .cmc-toggle{width:48px;min-width:48px;height:40px;}
      #${UI_ID}.collapsed .cmc-main{display:none;}
      #${UI_ID}.collapsed{left:auto;right:10px;}
      @media (orientation:landscape){
        #${UI_ID}{left:12px;right:12px;bottom:8px;}
        #${UI_ID} .cmc-dpad{width:142px;height:142px;}
        #${UI_ID} .cmc-mouse{width:190px;height:142px;}
      }
      @media (max-width:520px){
        #${UI_ID}{gap:6px;left:6px;right:6px;bottom:6px;}
        #${UI_ID} .cmc-dpad{width:132px;height:132px;}
        #${UI_ID} .cmc-key{width:44px;height:44px;}
        #${UI_ID} .cmc-w{left:44px}.cmc-a{left:0;top:44px}.cmc-s{left:44px;top:44px}.cmc-d{left:88px;top:44px;}
        #${UI_ID} .cmc-mouse{width:145px;height:132px;}
        #${UI_ID} button{min-width:46px;height:44px;}
      }
    `;
    ...
    const keyButtons = root.querySelectorAll('[data-key]');
    keyButtons.forEach((button) => {
      ...
      const press = (event) => {
        ...
        if (active) return;
        active = true;
        sendToGame({ source: MESSAGE_SOURCE, type: 'key', key, down: true });
      };
      const release = (event) => {
        event.preventDefault();
        if (!active) return;
        active = false;
        sendToGame({ source: MESSAGE_SOURCE, type: 'key', key, down: false });
      };
    ...
    mousePad.addEventListener('pointermove', (event) => {
      ...
      event.preventDefault();
      const dx = (event.clientX - lastX) * 1.7;
      const dy = (event.clientY - lastY) * 1.7;
      lastX = event.clientX;
      lastY = event.clientY;
      sendToGame({ source: MESSAGE_SOURCE, type: 'mouseMove', dx, dy });
    });
    const endMouse = (event) => {
      if (event.pointerId === mousePointerId) mousePointerId = null;
    };
    mousePad.addEventListener('pointerup', endMouse);
    mousePad.addEventListener('pointercancel', endMouse);

    root.querySelectorAll('[data-mouse]').forEach((button) => {
      const which = button.dataset.mouse;
      let active = false;
      const down = (event) => {
        event.preventDefault();
        if (active) return;
        active = true;
        sendToGame({ source: MESSAGE_SOURCE, type: 'mouseButton', button: which, down: true });
      };
      const up = (event) => {
        event.preventDefault();
        if (!active) return;
        active = false;
        sendToGame({ source: MESSAGE_SOURCE, type: 'mouseButton', button: which, down: false });
      };
    ...
    // Keep looking for the game iframe because itch.io can replace it after
    // loading or resizing. We don't access its DOM; cross-origin postMessage
    // is intentional and supported by the browser.
    let lastFrame = null;
    const findFrame = () => {
      const frames = [...document.querySelectorAll('iframe')];
      const frame = frames.find((f) => {
        try { return f.src.includes('html-classic.itch.zone/html/17576366/'); } catch (_) { return false; }
      });
      if (frame !== lastFrame) {
        lastFrame = frame || null;
        if (frame) frame.addEventListener('load', () => setTimeout(sendReady, 100));
      }
      return frame || null;
    };
    ...
    const sendReady = () => {
      ...
      if (frame && frame.contentWindow) {
        frame.contentWindow.postMessage({ source: MESSAGE_SOURCE, type: 'ping' }, GAME_ORIGIN);
      }
    };
    setInterval(sendReady, 1500);
    sendReady();
  }
  ...
  function sendToGame(message) {
    ...
    if (frame && frame.contentWindow) {
      frame.contentWindow.postMessage(message, GAME_ORIGIN);
    }
  }
})();
